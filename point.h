...
template <class T>point<T>::point(const point &rhs)
{
	if(this==&rhs)
		return *this;
	x=rhs.x;
...
template <class T> void point<T>::translate(T tx,T ty,T tz){
	this->x=x+tx;
	this->y=y+ty;
	this->z=z+tz;
}
template <class T> void point<T>::scale(T sx,T sy,T sz){
	this->x=x*sx;
	this->y=y*sy;
	this->z=z*sz;
}
template <class T> void point<T>::rotate(T xr,T yr,T theta){
	T rmatrix[4][4]={{cos(theta),-sin(theta),xr*(1-cos(theta))+yr*sin(theta)},
			{cos(theta),-sin(theta),xr*(1-cos(theta))+yr*sin(theta)},
			{cos(theta),-sin(theta),xr*(1-cos(theta))+yr*sin(theta)}}
	this->transform(rmatix);
}
